...
@click.option('--rect', '-rect', is_flag=True, help='Save as 4by3 rectangles (iris) instead of squares.')
def run_projection(
        ctx: click.Context,
        network_pkl: str,
        cfg: str,
    ...
    if save_every_step:
        ...
        PIL.Image.fromarray(synth_image[:,:,0], 'L').save(f'{result_name}_final.jpg')
...
#                 print(target_uint8[:,:,0].shape)
#                 break
    
                # video.append_data(np.concatenate([target_uint8, synth_image], axis=1))  # left side target, right projection
                video.append_data(np.concatenate([cv2.resize(target_uint8[:,:,0], (640,480)), 
                                                  cv2.resize(synth_image[:,:,0], (640,480))], axis=1))  # left side target, right projection
        else:
            for projected_w in projected_w_steps:
                synth_image = gen_utils.w_to_img(G, dlatents=projected_w, noise_mode='const')[0]
    
                # video.append_data(np.concatenate([target_uint8, synth_image], axis=1))  # left side target, right projection
                video.append_data(np.concatenate([target_uint8[:,:,0], synth_image[:,:,0]], axis=1))  # left side target, right projection
