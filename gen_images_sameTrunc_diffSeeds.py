...
def generate_images(
    ...
    # CUSTOM
    seeds_tmp = seeds[ (idx-1)*5000 : (idx)*5000 ]
#     seeds_tmp = seeds[ (idx-1)*10000 : (idx)*10000 ]

    if color == 1:

        # Generate images.
        for seed_idx, seed in enumerate(seeds_tmp):
            print('Generating image for seed %d (%d/%d) ...' % (seed, seed_idx, len(seeds_tmp))) # len(seeds) -> len(seeds_tmp)
    ...
    #             G.synthesis.input.transform.copy_(torch.from_numpy(m))
            ...
            PIL.Image.fromarray(img[0].cpu().numpy(), 'RGB').save('{}/seed{:07d}.png'.format(outdir, seed))
    ...
    #             G.synthesis.input.transform.copy_(torch.from_numpy(m))
            ...
            PIL.Image.fromarray(img[0], 'L').save('{}/seed{:07d}.png'.format(outdir, seed)) #L from RGB
